...
if(NOT "$ENV{OPENMS_PIXI_BUILD}" STREQUAL "")
  list(APPEND OWN_OPTIONS "-DOPENMS_PIXI_BUILD=$ENV{OPENMS_PIXI_BUILD}")
endif()
if(NOT "$ENV{CMAKE_BUILD_RPATH}" STREQUAL "")
  list(APPEND OWN_OPTIONS "-DCMAKE_BUILD_RPATH=$ENV{CMAKE_BUILD_RPATH}")
endif()
if(NOT "$ENV{CMAKE_INSTALL_RPATH}" STREQUAL "")
  list(APPEND OWN_OPTIONS "-DCMAKE_INSTALL_RPATH=$ENV{CMAKE_INSTALL_RPATH}")
endif()
if(NOT "$ENV{CMAKE_INSTALL_RPATH_USE_LINK_PATH}" STREQUAL "")
  list(APPEND OWN_OPTIONS "-DCMAKE_INSTALL_RPATH_USE_LINK_PATH=$ENV{CMAKE_INSTALL_RPATH_USE_LINK_PATH}")
endif()
...
function(_openms_print_file_tail path max_chars)
  if(EXISTS "${path}")
    file(READ "${path}" _content)
    string(LENGTH "${_content}" _len)
    if(_len GREATER ${max_chars})
      math(EXPR _start "${_len} - ${max_chars}")
      string(SUBSTRING "${_content}" ${_start} ${max_chars} _tail)
    else()
      set(_tail "${_content}")
    endif()
    message(STATUS "----- ${path} (tail) -----\n${_tail}\n----- end -----")
  else()
    message(STATUS "----- ${path} -----\n(file not found)\n----- end -----")
  endif()
endfunction()

if(NOT _configure_ret EQUAL 0)
  message(STATUS "CMake configure failed (ctest_configure return code: ${_configure_ret}).")
  _openms_print_file_tail("${CTEST_BINARY_DIRECTORY}/CMakeFiles/CMakeError.log" 12000)
  _openms_print_file_tail("${CTEST_BINARY_DIRECTORY}/CMakeFiles/CMakeOutput.log" 12000)
  _openms_print_file_tail("${CTEST_BINARY_DIRECTORY}/CMakeCache.txt" 12000)
  message(FATAL_ERROR "CMake configure failed; aborting before build.")
endif()
